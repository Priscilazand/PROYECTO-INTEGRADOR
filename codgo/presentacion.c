...
void printticket(char name[],int, float, char correo[15]);
...
void reservation()
{
    ...
    printf("\nIngrese el correo del paciente:> ");
	fflush(stdin);
	gets(passdetails.correo);
	...
	if(passdetails.esp_num>=1 && passdetails.esp_num<=10)
	{
		...
		printticket(passdetails.name,passdetails.esp_num,charges,passdetails.correo);
	}
	else
	{
	...
	if(confirm == 'y')
	{
        fprintf(fp,"NOMBRE PACIENTE\t\tNUMERO DE ESPECIALIDAD\t\tCORREO PACIENTE\t\tMONTO A PAGAR\n");
		fprintf(fp,"%s\t\t\t\t\t%d\t\t\t\t\t%s\t\t\t\t%.2f\n",&passdetails.name,passdetails.esp_num,passdetails.correo,charges);
		printf("==================");
		printf("\n CITA AGENDADA\n");
		printf("==================");
...
void printticket(char name[],int esp_num,float charges,char correo[15])
{
	system("cls");
	printf("-------------------\n");
